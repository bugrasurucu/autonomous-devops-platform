...
export default function RepositoriesPage() {
    ...
    // Demo repos — shown when GitHub is not connected
    const DEMO_REPOS: Repo[] = [
        { id: 1001, name: 'orbitron-api', fullName: 'orbitron/orbitron-api', description: 'NestJS backend API for the Orbitron platform. JWT auth, Prisma ORM, PostgreSQL.', language: 'TypeScript', stars: 234, private: false, defaultBranch: 'main', updatedAt: new Date(Date.now() - 3600000).toISOString(), url: 'https://github.com' },
        { id: 1002, name: 'orbitron-frontend', fullName: 'orbitron/orbitron-frontend', description: 'Next.js 14 App Router frontend. TailwindCSS, Shadcn UI, real-time agent monitoring.', language: 'TypeScript', stars: 189, private: false, defaultBranch: 'main', updatedAt: new Date(Date.now() - 7200000).toISOString(), url: 'https://github.com' },
        { id: 1003, name: 'ml-inference-service', fullName: 'acme/ml-inference-service', description: 'FastAPI ML inference microservice with GPU support, Redis caching, and OpenTelemetry.', language: 'Python', stars: 412, private: false, defaultBranch: 'main', updatedAt: new Date(Date.now() - 86400000).toISOString(), url: 'https://github.com' },
        { id: 1004, name: 'data-pipeline', fullName: 'acme/data-pipeline', description: 'Apache Kafka + Spark streaming pipeline for real-time analytics. Handles 10M events/day.', language: 'Python', stars: 87, private: true, defaultBranch: 'develop', updatedAt: new Date(Date.now() - 172800000).toISOString(), url: 'https://github.com' },
        { id: 1005, name: 'k8s-operator', fullName: 'acme/k8s-operator', description: 'Custom Kubernetes operator for managing stateful workloads with auto-scaling policies.', language: 'Go', stars: 341, private: false, defaultBranch: 'main', updatedAt: new Date(Date.now() - 259200000).toISOString(), url: 'https://github.com' },
        { id: 1006, name: 'payment-service', fullName: 'acme/payment-service', description: 'PCI-compliant payment processing service. Stripe, PayPal, webhook handling.', language: 'TypeScript', stars: 56, private: true, defaultBranch: 'main', updatedAt: new Date(Date.now() - 432000000).toISOString(), url: 'https://github.com' },
        { id: 1007, name: 'infra-modules', fullName: 'acme/infra-modules', description: 'Reusable Terraform modules for AWS VPC, ECS, RDS, and CloudFront. CIS benchmarks.', language: 'Dockerfile', stars: 128, private: false, defaultBranch: 'main', updatedAt: new Date(Date.now() - 518400000).toISOString(), url: 'https://github.com' },
        { id: 1008, name: 'analytics-dashboard', fullName: 'acme/analytics-dashboard', description: 'React + D3.js analytics dashboard with real-time WebSocket updates and PDF exports.', language: 'TypeScript', stars: 93, private: false, defaultBranch: 'main', updatedAt: new Date(Date.now() - 604800000).toISOString(), url: 'https://github.com' },
    ];

    if (!githubStatus.connected) {
        // Demo mode — show realistic repos without OAuth
        const demoFiltered = DEMO_REPOS.filter(r => {
            const matchSearch = !search || r.name.toLowerCase().includes(search.toLowerCase()) || (r.description || '').toLowerCase().includes(search.toLowerCase());
            const matchLang = langFilter === 'all' || r.language === langFilter;
            return matchSearch && matchLang;
        });
        const demoLangs = ['all', ...Array.from(new Set(DEMO_REPOS.map(r => r.language).filter(Boolean) as string[]))];
        ...
        return (
            <div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                    <div>
                        ...
                        <p style={{ color: 'var(--text-secondary)', fontSize: 13, marginTop: 4 }}>
                            Demo mode — 8 sample repositories · Connect GitHub for your real repos
                        </p>
                    </div>
                    <button className="btn-primary" onClick={connectGitHub} style={{ fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{
                    ...
                    <span>⚡</span>
                    <span>Demo mode active — deployments are real and will appear in your Pipeline. Connect GitHub to import your own repositories.</span>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: 16, height: 'calc(100vh - 220px)' }}>
                    ...
                    <div className="glass-card" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                        <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border-color)' }}>
                            <input className="input-field" placeholder="🔍 Search repositories..." value={search}
                            ...
                            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                                {demoLangs.map(lang => (
                                    <button key={lang} onClick={() => setLangFilter(lang)} style={{
                                        ...
                                        {lang !== 'all' && <span style={{ width: 7, height: 7, borderRadius: '50%', background: LANGUAGE_COLORS[lang] || '#8b949e', display: 'inline-block' }} />}
                                        {lang === 'all' ? `All (${DEMO_REPOS.length})` : lang}
                                    </button>
                                ))}
                            </div>
                        </div>
                        <div style={{ flex: 1, overflow: 'auto', padding: '8px' }}>
                            {demoFiltered.map(repo => (
                                <div key={repo.id} onClick={() => { setSelected(repo); setDeploySuccess(null); }} style={{
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                                        ...
                                        <div style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 11, color: 'var(--text-secondary)' }}>
                                            {repo.private && <span style={{ padding: '1px 5px', borderRadius: 4, background: 'rgba(251,191,36,0.1)', color: '#fbbf24', border: '1px solid rgba(251,191,36,0.2)' }}>Private</span>}
                                            {repo.language && (
                                    ...
                                    {repo.description && <p style={{ fontSize: 11, color: 'var(--text-secondary)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{repo.description}</p>}
                    ...
                    {!selected ? (
                        <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                            ...
                            <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>Select a repository to deploy</p>
                        </div>
                    ) : (
                        <div className="glass-card" style={{ padding: 28, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
                            <div>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                                    <h2 style={{ fontSize: 20, fontWeight: 700 }}><span className="gradient-text">{selected.name}</span></h2>
                                    <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 6, background: 'rgba(251,191,36,0.1)', color: '#fbbf24', border: '1px solid rgba(251,191,36,0.2)' }}>Demo</span>
                                </div>
                            ...
                            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                                {[
                                    ...
                                    <span key={badge.label} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 500, background: badge.color + '15', border: `1px solid ${badge.color}30`, color: badge.color }}>
                                        <span style={{ fontSize: 10 }}>{badge.icon}</span>{badge.label}
                                    </span>
                                ))}
                            </div>
                            <div style={{ padding: '10px 14px', borderRadius: 8, background: 'rgba(15,23,42,0.5)', border: '1px solid var(--border-color)', fontFamily: 'monospace', fontSize: 13 }}>
                                {selected.fullName}
                            </div>
                            ...
                            <div>
                                ...
                                {deploySuccess && (
                                    <div style={{ padding: '12px 16px', borderRadius: 8, marginBottom: 16, background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.3)', color: '#34d399', fontSize: 13, fontWeight: 600 }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                                    <div>
                                        ...
                                        <select className="input-field" value={deployConfig.region} onChange={e => setDeployConfig({ ...deployConfig, region: e.target.value })}>
                                    ...
                                    <div>
                                        ...
                                        <select className="input-field" value={deployConfig.environment} onChange={e => setDeployConfig({ ...deployConfig, environment: e.target.value })}>
                                ...
                                <div style={{ marginBottom: 16 }}>
                                    ...
                                    <input className="input-field" type="number" value={deployConfig.budget} onChange={e => setDeployConfig({ ...deployConfig, budget: Number(e.target.value) })} />
                                </div>
                                <div style={{ padding: '10px 14px', borderRadius: 8, border: '1px solid rgba(129,140,248,0.2)', background: 'rgba(129,140,248,0.04)', fontSize: 12, color: 'var(--text-secondary)', marginBottom: 16, lineHeight: 1.8 }}>
                                ...
                                <button className="btn-primary" onClick={handleDeploy} disabled={deploying} style={{ width: '100%', justifyContent: 'center' }}>
